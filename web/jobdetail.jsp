...
<div id="body">
    ...
    <div id="container">
                <div class="clearfix">
            <div class="content_l" id="content_l">
...
<script type="text/javascript" src="https://api.map.baidu.com/api?v=1.0&type=webgl&ak=PvxUSDMxOR25Fl0xXVIfLXKnKvi03AsI"></script>
<script type="text/javascript">
		//百度地图API功能
		var sMap = new BMapGL.Map("smallmap");
		sMap.enableScrollWheelZoom(true);
		
		if($('#positionLat').val() && $('#positionLng').val()){
			var sPoint = new BMapGL.Point($('#positionLng').val(),$('#positionLat').val());
			sMap.centerAndZoom(sPoint,12);
			sMap.addOverlay(new BMapGL.Marker(sPoint));              // 将标注添加到地图中
			
		}else{
			// 创建地址解析器实例
			var sMyGeo = new BMapGL.Geocoder();
			// 将地址解析结果显示在地图上,并调整地图视野
			sMyGeo.getPoint("武汉市武昌区螃蟹岬凤凰大厦A1-1301", function(sPoint){
			  if (sPoint) {
				  sMap.centerAndZoom(sPoint, 16);
				  sMap.addOverlay(new BMapGL.Marker(sPoint));
			  }
			}, "武汉");
		}
		...
		$(function(){
			$('#mapPreview').bind('click',function(){
			    ...
			    map.setZoom(12);
				if(lat && lng){
					var p = new BMapGL.Point(lng, lat);
					...
					gc.getPoint(address, function(point){
						  if (point) {
							  	var p = new BMapGL.Point(point.lng, point.lat);
								var marker = new BMapGL.Marker(p);  // 创建标注
								map.addOverlay(marker);              // 将标注添加到地图中
								setTimeout(function(){
									map.centerAndZoom(p, 15);
	...
	let p_id = '${p_id}';
	 $.ajax({
		 ...
		 success: function(msg){
			 ...
			 console.log(msg);
			 str1=str+" <dl class=\'job_detail\'>\r\n" + 
				"                    <dt>\r\n" + 
				"                        <h1 title=\'职位\'>\r\n" + 
				"                            <em></em>\r\n" + 
				"                                                        	<div>"+msg.msgobject.p_type+"</div>\r\n" + 
				"                           	                           	"+msg.msgobject.p_name+"\r\n" + 
				"                        </h1>\r\n" + 
				"                        \r\n" + 
				"                                               	\r\n" + 
				...
				"                    	<span class=\'red\'>"+msg.msgobject.p_salary+"</span>\r\n" + 
				"                       	<span>"+msg.msgobject.p_address+"</span> \r\n" + 
				"                       	<span>"+msg.msgobject.p_wexperience+" </span>\r\n" + 
				"                       	<span> "+msg.msgobject.p_graduation+"</span> <br>" + 
				"                      	  职位诱惑 : "+msg.msgobject.p_welfare+"\r\n" + 
				"                      	<div>联系电话："+msg.msgobject.p_tel+"</div>\r\n" + 
				"                    </dd>\r\n" + 
				"                    <dd class=\'job_bt\'>\r\n" + 
				"                        <h3 class=\'description\'>职位描述</h3>\r\n" + 
				"                        <p>公司描述： <br />"+msg.msgobject.p_info+" <br /></p>\r\n" + 
				"<p>岗位要求： <br />"+msg.msgobject.p_require+" </p>\r\n" + 
				"                    </dd>\r\n" + 
				"                     \r\n" + 
