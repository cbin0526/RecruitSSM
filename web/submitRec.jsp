            ...
            <div class="content_l">
            	<div class="fl" id="resume_name">
	            	<div class="nameShow fl">
	            		<h1 title="jason的简历">欢迎用户	${logincustomer.u_username}	</h1><br><br>
	            		<div id="MyDiv1"> </div>
	            		<button  onclick="removeRecord()">移除投递的简历</button>
	            		<h3>这是你的简历列表</h3>
	            	</div>	
            	</div><!--end #resume_name-->
             ...
             <button style="float:right; width: 100px; height: 50px" onclick="submitRecord()"><h3>投递简历</h3></button>
...
function myajax(page,size){
	  ...
	  $.ajax({
		 ...
		 success: function(msg){
			let str="";
			for(let i=0;i<msg.msgobject.recordlist.length;i++) 
			 {
			str=str+"<div class=\'profile_box\' id=\'basicInfo1\'>\r\n" + 
			" <h2>"+msg.msgobject.recordlist[i].r_recordname+"</h2>\r\n" + 
			"\r\n<div class=\'basicShow\'>\r\n" + 
			"<span>"+msg.msgobject.recordlist[i].r_name+" | "
			+msg.msgobject.recordlist[i].r_sex+" |   "
			+msg.msgobject.recordlist[i].r_workedyear+" |"
			+ msg.msgobject.recordlist[i].r_graduation+" <br>\r\n" 
			+msg.msgobject.recordlist[i].r_tel
			+ "| 岗位需求："+msg.msgobject.recordlist[i].r_position + "<br>\r\n" 
			+"</span>选择这个简历 <input type='radio' style='float: right;' name='mycheck' value='"+msg.msgobject.recordlist[i].r_recordid+"'>"
			+"\r\n\r\n</div>\r\n\r\n</div><p></p>"; 
		...
		var tools=' <a href="javascript:myajax(1,3)" id="myfirst">首页</a>&nbsp;&nbsp;&nbsp;&nbsp;'+
		    '<a href="javascript:myajax('+(msg.msgobject.page-1)+',3)">上一页</a>&nbsp;&nbsp;&nbsp;&nbsp;'+
		    '<span id="myspan"></span>'+
	        '<a href="javascript:myajax('+(msg.msgobject.page+1)+',3)">下一页</a>&nbsp;&nbsp;&nbsp;&nbsp;'+
	        '<a href="javascript:myajax('+(msg.msgobject.maxPage)+',3)">尾页</a>&nbsp;&nbsp;&nbsp;&nbsp;';	
		
	        //将首页，尾页，上一页，下一页注入到对应的div中
	        $("#mypagetool").html(tools);
	        
	    	//获取当前页以及最大页数
	    	let page=msg.msgobject.page;
	    	let maxPage=msg.msgobject.maxPage;
	    	let str1="";
	    	let str2="";
	    	if(page!=''&&maxPage!=''){
	    		page=parseInt(page);
	    		...
	    		for(let i=1;i<=maxPage;i++){
	    			if(i==page){
	    				str1=str1+"第"+i+"页</a>&nbsp;&nbsp;";
	    				str2=str2+"<option selected='selected' value='"+i+"'>第"+i+"页</option>";
	    			}else{				
	    				str1=str1+"<a href='javascript:myajax("+i+",3)'>第"+i+"页</a>&nbsp;&nbsp;"
	    				str2=str2+"<option  value='"+i+"'>第"+i+"页</option>";
	    			}
	    		;	
	    		}
	    		$("#myspan").html(str1);
	    		$("#myselected").html(str2);
	    	}   
	        
	     
	    	
	    	$("#myfisrt").attr("href","javascript:myajax(1,3)");
	    	$("#myend").attr("href","javascript:myajax("+msg.msgobject.maxPage+",3)"); 
	    	$("#mylast").attr("href","javascript:myajax("+(msg.msgobject.page-1)+",3)");
	    	$("#mynext").attr("href","javascript:myajax("+(msg.msgobject.page+1)+",3)");
		 }	  
	  });	
	}
...
<script type="text/javascript">
	...
	function submitRecord(){
		//获取简历id
		let r_recordid = $('input:radio:checked').val();
		//获取岗位id
		let p_id = '${p_id}';
		//获取用户id
		let u_userid = '${logincustomer.u_userid}';
		if(r_recordid === undefined){
			alert("请选择要投递的简历")
		}else{
			$.ajax({
				url:"SubmitRecord.do",
	...
	$.ajax({
		...
		success:function(msg){
			...
			if(msg){
				...
				if(msg.rs_status == 1){
					status = '已经查看';
				}else if(msg.rs_status == 0){
					status = '未查看';
				}else if(msg.rs_status == 3){
					status = '未通过';
				}
				str=str+"<h3>已经投递的简历：</h3><div class=\'profile_box\' >\r\n" + 
				" <h2>"+msg.r_name+"</h2>" + 
				"\r\n<div class=\'basicShow\'>\r\n" + 
				"<span>"+msg.r_name+" | "
				+msg.r_sex+" |   "
				+msg.r_workedyear+" |"
				+ msg.r_graduation+"|&nbsp&nbsp查看状态：<span>"+status+"</span> <br>\r\n" 
				+msg.r_tel
				+ "| 应聘岗位："+msg.r_position + "<br>\r\n" 
				+"\r\n\r\n</div>\r\n\r\n</div><p></p>";
			}
			else{
				str=str+"<h3>没有投递简历：</h3>"
			}
			$("#MyDiv1").html(str);
		}
	})
...
<script type="text/javascript">
	function removeRecord(){
		let flag = confirm("你确定要移除已经提交的简历吗？");
		if(flag){
			//获取岗位id
			let p_id = '${p_id}';
			//获取用户id
